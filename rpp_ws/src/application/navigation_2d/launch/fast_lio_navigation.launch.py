...
def get_nav2_params():
    ...
    map_yaml = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            os.environ["HOME"], "rpp_data/map/fast_lio_sam/map.yaml"
        ),  # todo:可能有问题
        description="地图的路径",
    )
    ...
    declare_params_fild = DeclareLaunchArgument(
        "params_file",
        default_value=os.path.join(robot_bringup_path, "params", "nav2_3d.yaml"),
    ...
    publish_odom = DeclareLaunchArgument(
        "publish_odom", default_value="false", description=""
    )
    publish_odom_tf = DeclareLaunchArgument(
        "publish_odom_tf", default_value="false", description=""
    )
...
def create_equipment_and_rviz_nodes(
    namespace, use_namespace, rviz_config, use_rviz, publish_odom, publish_odom_tf
):
    ...
    base_tf = Node(
        ...
        arguments=["-0.066,", "0", "-0.345", "0", "0", "0", "1", "livox", "base_link"],
    )
    ...
    return [lidar_launch_file, rviz_launch_file, base_tf]
...
def create_nav2_nodes(
    ...
    return [navigation_node, nav2_container_node]


def create_localization_nodes(
    namespace,
    map_yaml_file,
    use_sim_time,
    autostart,
...
def generate_launch_description():
    ...
    localization_nodes = create_localization_nodes(
        namespace=namespace,
    ...
    for node in nav2_params + robot + nav2_nodes + localization_nodes:
        ld.add_action(node)
    return ld
