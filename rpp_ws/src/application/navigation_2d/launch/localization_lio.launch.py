...
def get_nav2_params():
    ...
    lio_localizer_file = DeclareLaunchArgument(
...
def create_composable_nodes(
    ...
    localizer_nodes = create_localizer_nodes(
        ...
        use_sim_time=use_sim_time,
        lio_config_path=lio_config_path,  # 1
        localizer_config_path=localizer_config_path,
        remappings=remappings,
    )  #
    load_nodes = GroupAction(actions=localizer_nodes)
    return [load_nodes]
...
def generate_launch_description():
    ...
    lio_config_path = LaunchConfiguration("lio_config_file")
    ...
    for node in nav2_params + composable_nodes + stdout_linebuf_envvar:
        ld.add_action(node)
    return ld
