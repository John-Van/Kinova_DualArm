...
def get_nav2_params():
    ...
    namespace = DeclareLaunchArgument(
        "namespace", default_value="", description="\默认为根命名空间"
    )
    ...
    map_yaml = DeclareLaunchArgument(
        "map",
        default_value=os.path.join("/home/rpp/rpp_data/map/fast_lio_sam/map.yaml"),
        description="地图的路径",
    )
    ...
    container_name = DeclareLaunchArgument(
        ...
        description="如果使用组合启动，节点将加载的容器的名称",
    )
...
def create_nav2_nodes(
    ...
    nav2_collision_monitor_node = Node(
...
def create_load_composable_nodes(
    use_composition,
    container_name_full,
    remappings,
    configured_params,
...
def generate_launch_description():
    ...
    load_composable_nodes = create_load_composable_nodes(
        use_composition=use_composition,
    ...
    for node in (
        nav2_params + composable_nodes + load_composable_nodes + stdout_linebuf_envvar
    ):
        ld.add_action(node)
    return ld
