...
struct NodeConfig
{
    ...
    std::string pcd_path = "/home/hfsh/map/map.pcd";
...
public:
    LocalizerNode() : Node("localizer_node")
    {
        ...
        m_initalpose_sub = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>("/initialpose", 10, std::bind(&LocalizerNode::initialPoseCB, this, std::placeholders::_1));
        ...
        {
            m_localizer->loadMap(m_config.pcd_path);
            RCLCPP_INFO(this->get_logger(), "load map from %s", m_config.pcd_path.c_str());
        }
        else
        {
            RCLCPP_WARN(this->get_logger(), "map not found, please check the path: %s", m_config.pcd_path.c_str());
        }
    }

    void loadParameters()
    {
        this->declare_parameter("config_path", "/home/hfsh/fastlio2_ws/src/FASTLIO2_ROS2/localizer/config/localizer.yaml");
    ...
    void initialPoseCB(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
    {
        ...
        initial_guess.block<3, 3>(0, 0) = (yawAngle * rollAngle * pitchAngle).toRotationMatrix().cast<float>();
        initial_guess.block<3, 1>(0, 3) = V3F(msg->pose.pose.position.x, msg->pose.pose.position.y, msg->pose.pose.position.z);
        std::lock_guard<std::mutex>(m_state.service_mutex);
        m_state.initial_guess = initial_guess;
...
private:
    ...
    rclcpp::Subscription<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr m_initalpose_sub;
};
