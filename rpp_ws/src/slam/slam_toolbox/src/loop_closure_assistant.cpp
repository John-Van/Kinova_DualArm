...
: mapper_(mapper), scan_holder_(scan_holder),
  ...
  processor_type_(processor_type),base_frame_(base_frame)
/*****************************************************************************/
{
...
/*****************************************************************************/
{
      ...
      visualization_msgs::msg::InteractiveMarkerFeedback::POSE_UPDATE)
  {
    ...
    // Add tf from base_link to scan
    std::shared_ptr<tf2_ros::Buffer> tf_buffer = std::make_shared<tf2_ros::Buffer>(node_->get_clock());
    tf2_ros::TransformListener tf_listener(*tf_buffer);
    geometry_msgs::msg::TransformStamped t;
    try {
        t = tf_buffer->lookupTransform(base_frame_,scan.header.frame_id.c_str(),scan.header.stamp,rclcpp::Duration(1, 0));
    } catch (tf2::TransformException &ex) {
        RCLCPP_WARN(node_->get_logger(), "Could not transform base_link to %s: %s", scan.header.frame_id.c_str(), ex.what());
        return;
    }

    // Convert geometry_msgs::msg::Transform to tf2::Transform
    tf2::Transform scan_to_base;
    tf2::fromMsg(t.transform, scan_to_base);

    // publish the scan visualization with transform
    geometry_msgs::msg::TransformStamped msg;
    tf2::convert(scan_to_base*transform, msg.transform);
